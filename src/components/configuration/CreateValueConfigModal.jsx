...
export default function CreateValueConfigModal({ open, onClose, onSuccess }) {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      await apiClient.configuration.createValueConfiguration({
      ...
      toast.success('Configuración de valores creada correctamente');
      reset();
      setRules([{ minValue: 0, maxValue: 100, valuation: 'Sin problema', color: '#22c55e' }]);
      onSuccess();
      onClose();
    } catch (error) {
      console.error('Error creating value configuration:', error);
      toast.error(
        error.response?.data?.error?.message || 'Error al crear la configuración de valores'
      );
    } finally {
      setSubmitting(false);
    }
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Crea una configuración de valores para un test con reglas de valoración
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <DialogFooter>
            ...
            >
              {submitting ? 'Creando...' : 'Crear Configuración'}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
