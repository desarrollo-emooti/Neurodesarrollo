...
export default function ImportTemplateEditForm({ open, onClose, template, onSuccess }) {
  ...
  useEffect(() => {
    if (open && template) {
      ...
      setValue('templateType', template.templateType || 'pruebas');
      setValue('relatedTestId', template.relatedTestId || '');
  ...
  const onSubmit = async (data) => {
    try {
      ...
      await apiClient.configuration.updateImportTemplate(template.id, {
        name: data.name.trim(),
      ...
      toast.success('Plantilla de importación actualizada correctamente');
      onSuccess();
      onClose();
    } catch (error) {
      console.error('Error updating import template:', error);
      toast.error(
        error.response?.data?.error?.message || 'Error al actualizar la plantilla de importación'
      );
    } finally {
      setSubmitting(false);
    }
      ...
      <DialogContent className="sm:max-w-[900px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Editar Plantilla de Importación
          </DialogTitle>
          <DialogDescription>
            Modifica la plantilla de importación de datos
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              {/* Template Type (disabled - cannot change) */}
              <div className="space-y-2">
                <Label htmlFor="templateType">
                  Tipo <span className="text-red-500">*</span>
                </Label>
                <Select value={templateType} disabled>
                ...
                <p className="text-xs text-slate-500">El tipo de plantilla no se puede cambiar</p>
              </div>
